...
while time < tfinal:
    ...
    v3.set_valve_coeff(critical_flow(gas, cchamberP, gas.T, gas.mean_molecular_weight, kappa(gas), exaustP, 5e-3))
    time = sim.step()


    v = (2 * (kappa(gas) * ct.gas_constant / gas.mean_molecular_weight) / (kappa(gas) - 1) * gas.T * (
    1 - (exaustP / gas.P)) ** ((kappa(gas) - 1) / kappa(gas))) ** 0.5
    states.append(gas.state, t=time, mass=cchamber.mass, v=v)
    print(round(time, 5), round(gas.P / 1e6, 3), round(gas.T - 273, 3), round(cchamber.mass*1000, 3), v)
...
plt.xlabel('$t[s]$')
plt.ylabel('$[-]$')
plt.plot(states.t, states('H2').Y, label='H2')
